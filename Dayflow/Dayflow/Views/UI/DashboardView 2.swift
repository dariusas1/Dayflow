...
// MARK: - Supporting Types for Dashboard
...
extension TrendInsight: StringProtocol {
    // Minimal StringProtocol conformance to fix Text(insight) usage
    ...
    subscript(bounds: Range<String.Index>) -> SubSequence {
        String(text[bounds])
    }
}
...
struct KeyMetricsRow: View {
    ...
    private func getTrendForMetric(_ category: ProductivityMetric.MetricCategory, trends: [TrendData]) -> TrendDirection {
        return trends.first { $0.metricName.contains(category.rawValue) }?.trendDirection ?? TrendDirection.stable
    }
}
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if !trend.insights.isEmpty {
                ForEach(Array(trend.insights.prefix(2)), id: \.id) { insight in
                    HStack {
                        Circle()
...
struct MiniRecommendationCard: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            PriorityIndicator(priority: Recommendation.Priority(from: recommendation.priority))
        }
        .padding(12)
        .background(Color.white)
        .cornerRadius(8)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if metrics.isEmpty {
                ...
                let totalTasks = metrics.reduce(0) { $0 + $1.value }
                Text("\(Int(totalTasks)) tasks completed")
                    .font(.custom("Nunito", size: 12))
...
struct WidgetProductivityScore: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            let latestScore = metrics.first { $0.category == .productivity }
            Text(latestScore != nil ? "\(Int(latestScore!.value * 100))%" : "--")
                .font(.custom("Nunito", size: 20))
                .fontWeight(.bold)
...
struct WidgetFocusTimeChart: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            let todayFocus = metrics.first { $0.category == .focusTime && Calendar.current.isDateInToday($0.timestamp) }
            Text(todayFocus != nil ? "\(Int(todayFocus!.value)) min" : "0 min")
                .font(.custom("Nunito", size: 20))
                .fontWeight(.bold)
                .foregroundColor(.blue)
        }
...
extension Recommendation.Priority {
    var color: Color {
        switch self {
                ...
                .fill(
                    LinearGradient(
                        gradient: Gradient(colors: [
                            trend.trendDirection.color.opacity(0.3),
                            trend.trendDirection.color.opacity(0.1)
                        ]),
                        startPoint: .leading,
                        endPoint: .trailing
                    )
...
extension Recommendation.Priority {
    init(from plannerPriority: PlannerPriority) {
